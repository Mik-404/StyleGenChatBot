...
async def style_gen(message: types.Message):
    ...
    try:
        if len(message.text.split()) == 2 \
            ...
            msg_type = await db.db_get_style(message.chat.id, INFO_group, message.text.split()[-1])
...
@dp.message_handler(handler_filters.CommandFilter(), commands=['set_style'], content_types=types.ContentType.TEXT)
async def set_style(message: types.Message):
    logging = logs.BotLogReplicas(message)
    await logging.start_set_style_processing()
    try:
        if (len(message.text.split()) == 2 or len(message.text.split()) == 3) \
                and message.text.split()[-1] in settings.STYLES:
...
@dp.message_handler(handler_filters.CommandFilter(), commands=['set_prob'], content_types=types.ContentType.TEXT)
...
@dp.message_handler(handler_filters.CommandFilter(), commands=['help', 'start'], content_types=types.ContentType.TEXT)
async def help_for_user(message: types.Message):
    await message.reply(texts.help_massage)


@dp.message_handler(handler_filters.CommandFilter(), commands=['status'], content_types=types.ContentType.TEXT)
